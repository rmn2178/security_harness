'use strict';
// nosih-scan/scanner.js — NOSIH Labs | Apache-2.0
...
function scanTools(tools, serverId = 'unknown') {
  ...
  if (byRisk.CRITICAL > 0) recommendations.push('Add NOSIH policy bundle with human_review_triggers on T3 tools');
  ...
  recommendations.push('Full governance: npx nosih-scan --json | submit to nosih.gg/scan');
...
function formatReport(report) {
  console.log(`\n${BOLD}${CYAN}══════════════════════════════════════════${RESET}`);
  console.log(`${BOLD}  NOSIH MCP Security Scanner${RESET}  ${DIM}nosih.gg${RESET}`);
  ...
  console.log(`\n${DIM}  Full governance layer: https://nosih.gg${RESET}`);
  console.log(`${DIM}  Star: https://github.com/nosih-ai/nosih-protocol${RESET}\n`);
}
