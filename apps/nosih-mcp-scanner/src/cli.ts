#!/usr/bin/env node
/**
 * NOSIH MCP Security Scanner — CLI entry point.
 *
 * Usage:
 *   echo '[{"name":"readFile","description":"reads a file"}]' | nosih-scan --server filesystem
 *   nosih-scan --server myserver --tools '[{"name":"bash","description":"runs shell commands"}]'
 */
...
function printReport(report: ServerScanReport): void {
  console.log(`\n${BOLD}${CYAN}══════════════════════════════════════════════${RESET}`);
  console.log(`${BOLD} NOSIH MCP Security Scanner Report${RESET}`);
  ...
  if (report.byRisk.HIGH > 0 || report.byRisk.CRITICAL > 0) {
    console.log(`${BOLD}${MAGENTA}One-Click NOSIH Proxy Scaffold:${RESET}`);
...
function printClaudeConfigSnippet(serverId: string): void {
  const proxyId = `nosih-${serverId}`;
  const snippet = {
    mcpServers: {
      [proxyId]: {
        command: "node",
        args: ["/absolute/path/to/nosih-protocol/apps/nosih-mcp/dist/index.js"],
        env: {
          NOSIH_UPSTREAM_COMMAND: "npx",
          NOSIH_UPSTREAM_ARGS: `-y @modelcontextprotocol/server-${serverId}`,
          NOSIH_MAX_TIER: "T2_ACT",
          NOSIH_REQUIRE_APPROVAL_TIER: "T3_COMMIT",
        },
      },
    },
  };
...
async function main(): Promise<void> {
  ...
  let registryUrl = "https://registry.nosih.gg/v1";
  ...
  if (!toolsJson) {
    ...
    console.error(
      'Usage: echo \'[{"name":"readFile","description":"reads a file"}]\' | nosih-scan --server filesystem',
    );
    process.exit(1);
  }
