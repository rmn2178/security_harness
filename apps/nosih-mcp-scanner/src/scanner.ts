/**
 * NOSIH MCP Security Scanner — analyzes MCP server tool definitions for risk.
 *
 * Maps tool annotations and descriptions to NOSIH approval tiers and
...
function generateRecommendations(tools: ToolScanResult[]): string[] {
  ...
  if (criticalCount > 0 || highCount > 0) {
    ...
    recs.push("Enable NOSIH capability tokens with strict resource scoping for high-risk tools.");
  }
  if (tools.length === 0) {
