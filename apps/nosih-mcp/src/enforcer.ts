/**
 * NOSIH MCP — Policy Enforcement Layer.
 *
 ...
 * to NOSIH requests and routes them through the PolicyGateway.
 *
 * @module @nosih/mcp/enforcer
 */

import { type NosihRequest, type PolicyDecision, ApprovalTier } from "@pshkv/core";
...
import { toResourceUri, toNosihAction } from "@pshkv/bridge-mcp";
...
/**
 * Policy Enforcer — intercepts every tool call through NOSIH.
 *
 * @example
 * ```ts
...
export class PolicyEnforcer {
  ...
  /**
   * Enforce NOSIH policy on a tool call.
   *
   * Flow:
   * 1. Map tool call to NosihRequest
  ...
  async enforce(
    ...
    switch (decision.action) {
      ...
      case "deny": {
        const reason = decision.denial?.reason ?? "Denied by NOSIH policy";
  ...
  private async handleEscalation(
    ...
    const approvalReq = this.approvalQueue.enqueue(nosihRequest, decision);
    ...
    return {
      allowed: false,
      decision,
      denyReason: `Escalated: ${reason}. Use nosih__approve to approve pending actions.`,
      approvalRequestId: approvalReq.requestId,
    };
  }
