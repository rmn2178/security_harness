/**
 * NOSIH MCP — Multi-Agent Delegation Tools.
 *
 * Provides nosih__delegate_to_agent, nosih__list_delegations, and
 * nosih__revoke_delegation_tree for hierarchical multi-agent orchestration.
 *
...
export function getDelegationToolDefinitions(): Array<{
  ...
  return [
      ...
      inputSchema: {
        ...
        properties: {
          ...
          toolScope: {
            ...
            minItems: 1,
            examples: [["mcp://filesystem/reports/*"], ["mcp://github/repos/nosih-ai/nosih-protocol/issues/*"]],
          },
          expiresInHours: {
            title: "Expiry (hours)",
            type: "number",
    ...
    {
      name: "nosih__revoke_delegation_tree",
      description:
...
export async function handleDelegationTool(
  ...
  switch (toolName) {
    case "nosih__delegate_to_agent":
      return handleDelegate(args, ctx);
    case "nosih__list_delegations":
      return handleListDelegations(ctx);
    case "nosih__revoke_delegation_tree":
...
function handleRevokeTree(args: Record<string, unknown>, ctx: DelegationToolContext) {
  ...
  const reason = (args["reason"] as string | undefined) ?? "Revoked via nosih__revoke_delegation_tree";
