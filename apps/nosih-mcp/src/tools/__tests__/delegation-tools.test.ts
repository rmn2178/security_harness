/**
 * NOSIH MCP — Delegation tools tests.
 *
 * Covers nosih__delegate_to_agent, nosih__list_delegations,
 * and nosih__revoke_delegation_tree.
 */
...
describe("isDelegationTool", () => {
  it("returns true for known delegation tool names", () => {
    expect(isDelegationTool("nosih__delegate_to_agent")).toBe(true);
    expect(isDelegationTool("nosih__list_delegations")).toBe(true);
    expect(isDelegationTool("nosih__revoke_delegation_tree")).toBe(true);
  });

  it("returns false for other tool names", () => {
    expect(isDelegationTool("nosih__status")).toBe(false);
    expect(isDelegationTool("nosih__issue_token")).toBe(false);
...
describe("getDelegationToolDefinitions", () => {
  ...
  it("contains the three expected tool names", () => {
    ...
    expect(names).toContain("nosih__delegate_to_agent");
    expect(names).toContain("nosih__list_delegations");
    expect(names).toContain("nosih__revoke_delegation_tree");
  });
});

describe("nosih__delegate_to_agent", () => {
...
describe("nosih__list_delegations", () => {
  it("returns empty message when no delegations", async () => {
    const ctx = createContext();
    const result = await handleDelegationTool("nosih__list_delegations", {}, ctx);
  ...
  it("returned nodes include expected fields", async () => {
    ...
    await handleDelegationTool(
      "nosih__delegate_to_agent",
      { subagentId: subKeypair.publicKey, toolScope: ["mcp://nosih/*"] },
      ctx,
    );
    ...
    expect(node.toolScope).toEqual(["mcp://nosih/*"]);
...
describe("nosih__revoke_delegation_tree", () => {
  it("returns error when rootTokenId is missing", async () => {
    const ctx = createContext();
    const result = await handleDelegationTool("nosih__revoke_delegation_tree", {}, ctx);
    expect(result.content[0]!.text).toContain("Error");
    expect(result.content[0]!.text).toContain("required");
  });
  ...
  it("uses default reason when reason not provided", async () => {
    ...
    const parsed = JSON.parse(result.content[0]!.text);
    expect(parsed.reason).toBe("Revoked via nosih__revoke_delegation_tree");
  });
});
...
describe("handleDelegationTool unknown tool", () => {
  it("returns error for unknown tool name", async () => {
    const ctx = createContext();
    const result = await handleDelegationTool("nosih__unknown_tool", {}, ctx);
