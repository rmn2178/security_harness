/**
 * NOSIH MCP — Built-in NOSIH Tools.
 *
 * These tools are prefixed with "nosih__" and provide NOSIH-specific
 * functionality directly as MCP tools: status, approval workflow,
 * audit trail, server management.
 *
 * @module @nosih/mcp/tools/nosih-tools
 */
...
export function getNosihToolDefinitions(): Array<{
  name: string;
  description: string;
  ...
  return [
    {
      name: "nosih__status",
      description:
        "Inspect the current NOSIH runtime state before taking action. Use this to confirm the server is healthy, see how many downstream servers are connected, and check pending approvals. Returns a JSON status summary with server counts, aggregated tools, pending approvals, and ledger size.",
      inputSchema: { type: "object", properties: {}, required: [], additionalProperties: false },
      annotations: { title: "NOSIH Status", readOnlyHint: true, idempotentHint: true, openWorldHint: false },
    },
    {
      name: "nosih__servers",
      description:
    ...
    },
    {
      name: "nosih__whoami",
      description:
        "Show the active NOSIH identity for the current session. Use this before issuing tokens, approving requests, or debugging delegation so you can confirm the acting public key and token context. Returns a JSON object with the current public key, token ID, and role.",
      inputSchema: { type: "object", properties: {}, required: [], additionalProperties: false },
    ...
    },
    {
      name: "nosih__pending",
      description:
        "List approval requests that are blocked waiting for review. Use this before calling nosih__approve or nosih__deny so you can inspect request IDs, affected resources, actions, reasons, and expiration times. This tool does not mutate state and returns a JSON array of pending approval summaries.",
      inputSchema: {
        type: "object",
        properties: {},
        required: [],
    ...
    },
    {
      name: "nosih__approve",
      description:
        "Approve one pending escalated action after human or operator review. Use this only with a requestId returned by nosih__pending; approval releases the blocked action for execution and records the approver identity. Returns a confirmation message, or an error if the request does not exist.",
      inputSchema: {
        type: "object",
        properties: {
          requestId: {
            ...
            type: "string",
            description: "Approval request ID returned by nosih__pending",
            minLength: 1,
            examples: ["apr_01hxyz..."],
          },
          by: {
    ...
    {
      name: "nosih__add_server",
      description:
        "Register and connect a new downstream MCP server while NOSIH is running. Use this to aggregate a new stdio server into the proxy without restarting the process; on success the server is immediately available through the aggregated tool namespace. Returns a confirmation message including the discovered tool count, or an error message if connection fails.",
      inputSchema: {
        type: "object",
        properties: {
          name: {
      ...
      inputSchema: {
        ...
        properties: {
          name: {
            title: "Server name",
            type: "string",
            description: "Server name as listed by nosih__servers",
            minLength: 1,
            examples: ["filesystem", "github"],
          },
        },
      ...
      inputSchema: {
        ...
        properties: {
          ...
          resource: {
            ...
            minLength: 1,
            examples: ["mcp://filesystem/*", "mcp://github/repos/nosih-ai/*"],
          },
          actions: {
        ...
        additionalProperties: false,
        examples: [{ subject: "agent:planner-01", resource: "mcp://github/repos/nosih-ai/nosih-protocol/*", actions: ["call"], expiresInHours: 24 }],
      },
      annotations: { title: "Issue Capability Token", destructiveHint: false, idempotentHint: false, openWorldHint: false },
    },
    {
      name: "nosih__revoke_token",
      description:
...
/** Check if a tool name is a built-in NOSIH tool. */
export function isNosihTool(name: string): boolean {
  return name.startsWith("nosih__");
}

/** Context needed by NOSIH tool handlers. */
export interface NosihToolContext {
...
/**
 * Handle a built-in NOSIH tool call.
 */
export async function handleNosihTool(
  toolName: string,
  args: Record<string, unknown>,
  ctx: NosihToolContext,
  ...
  switch (toolName) {
    ...
    case "nosih__add_server":
      return handleAddServer(args, ctx);
    case "nosih__remove_server":
      return handleRemoveServer(args, ctx);
    case "nosih__issue_token":
      return handleIssueToken(args, ctx);
    case "nosih__revoke_token":
      return handleRevokeToken(args, ctx);
    default:
      return text(`Unknown NOSIH tool: ${toolName}`);
  }
}
...
function handleStatus(ctx: NosihToolContext) {
...
function handleServers(ctx: NosihToolContext) {
  const servers = ctx.downstream.listServers();
  return text(JSON.stringify(servers, null, 2));
}

function handleWhoami(ctx: NosihToolContext) {
  return text(JSON.stringify({
...
function handlePending(ctx: NosihToolContext) {
...
function handleApprove(args: Record<string, unknown>, ctx: NosihToolContext) {
...
function handleDeny(args: Record<string, unknown>, ctx: NosihToolContext) {
  const requestId = args["requestId"] as string | undefined;
  if (!requestId) {
    return text("Error: requestId is required");
  }
  ...
  const by = (args["by"] as string | undefined) ?? ctx.agentPublicKey.slice(0, 16);
  const reason = (args["reason"] as string | undefined) ?? "Denied via nosih__deny";
  const resolution = ctx.approvalQueue.resolve(requestId, {
    status: "denied",
    by,
    reason,
...
function handleAudit(args: Record<string, unknown>, ctx: NosihToolContext) {
...
async function handleAddServer(args: Record<string, unknown>, ctx: NosihToolContext) {
...
async function handleRemoveServer(args: Record<string, unknown>, ctx: NosihToolContext) {
  const name = args["name"] as string | undefined;
  if (!name) {
...
function handleIssueToken(args: Record<string, unknown>, ctx: NosihToolContext) {
...
function handleRevokeToken(args: Record<string, unknown>, ctx: NosihToolContext) {
  const tokenId = args["tokenId"] as string | undefined;
  const reason = (args["reason"] as string | undefined) ?? "Revoked via nosih__revoke_token";
