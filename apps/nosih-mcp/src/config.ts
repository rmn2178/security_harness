...
/** Full NOSIH MCP configuration. */
export interface NosihMCPConfig {
 ...
 */
export function loadConfig(argv: string[] = process.argv.slice(2)): NosihMCPConfig {
  // Parse CLI args
  const cliArgs = parseCliArgs(argv);

  // Load config file
  const configPath = cliArgs.config
    ?? process.env["NOSIH_MCP_CONFIG"]
    ?? findConfigFile();

  let fileConfig: Partial<NosihMCPConfig> = {};
  if (configPath && existsSync(configPath)) {
    const raw = readFileSync(configPath, "utf-8");
    try {
      fileConfig = JSON.parse(raw) as Partial<NosihMCPConfig>;
    } catch (err) {
      throw new Error(
        `Failed to parse NOSIH MCP config file "${configPath}": ${err instanceof Error ? err.message : String(err)}.\n` +
        `Ensure it is valid JSON. See nosih-mcp.config.example.json for reference.`,
      );
    }
  }
  ...
  return {
    ...
    defaultPolicy: (
      ...
      ?? process.env["NOSIH_MCP_POLICY"]
      ?? fileConfig.defaultPolicy
      ?? DEFAULTS.defaultPolicy
    ) as NosihMCPConfig["defaultPolicy"],
    approvalTimeoutMs: Number(
      cliArgs.approvalTimeoutMs
      ?? process.env["NOSIH_MCP_APPROVAL_TIMEOUT"]
      ?? fileConfig.approvalTimeoutMs
      ?? DEFAULTS.approvalTimeoutMs,
    ),
    transport: (
      cliArgs.transport
      ?? process.env["NOSIH_MCP_TRANSPORT"]
      ?? fileConfig.transport
      ?? DEFAULTS.transport
    ) as NosihMCPConfig["transport"],
    port: Number(
      cliArgs.port
      ?? process.env["NOSIH_MCP_PORT"]
      ?? fileConfig.port
      ?? DEFAULTS.port,
    ),
    agentPrivateKey:
      cliArgs.agentPrivateKey
      ?? process.env["NOSIH_AGENT_PRIVATE_KEY"]
...
function findConfigFile(): string | undefined {
  const candidates = [
    resolve(process.cwd(), "nosih-mcp.config.json"),
    resolve(process.cwd(), ".nosih-mcp.json"),
  ];
