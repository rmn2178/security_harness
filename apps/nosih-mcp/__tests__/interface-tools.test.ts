/**
 * NOSIH MCP — Interface tools tests.
 *
 * Tests all nosih__interface_* operator tools:
...
describe("isInterfaceTool", () => {
  ...
  it("returns false for downstream and nosih tools", () => {
    expect(isInterfaceTool("filesystem__readFile")).toBe(false);
    expect(isInterfaceTool("nosih__approve")).toBe(false);
...
describe("nosih__interface_status", () => {
  it("returns current interface state as JSON", async () => {
    const ctx = createCtx();
    const result = await handleInterfaceTool("nosih__interface_status", {}, ctx);
...
describe("nosih__speak", () => {
  it("speaks with default priority", async () => {
    const ctx = createCtx();
    const result = await handleInterfaceTool("nosih__speak", { text: "Hello operator" }, ctx);
  ...
  it("speaks with urgent priority", async () => {
    const ctx = createCtx();
    const result = await handleInterfaceTool("nosih__speak", { text: "STOP!", priority: "urgent" }, ctx);
  ...
  it("speaks with low priority", async () => {
    const ctx = createCtx();
    const result = await handleInterfaceTool("nosih__speak", { text: "FYI", priority: "low" }, ctx);
  ...
  it("emits voice output ledger event", async () => {
    const ctx = createCtx();
    await handleInterfaceTool("nosih__speak", { text: "Test voice" }, ctx);
  ...
  it("returns error when text is missing", async () => {
    const ctx = createCtx();
    const result = await handleInterfaceTool("nosih__speak", {}, ctx);
    expect(result.content[0]!.text).toContain("text is required");
  });
});

describe("nosih__show_hud", () => {
  ...
  it("updates an audit panel", async () => {
    const ctx = createCtx();
    const result = await handleInterfaceTool("nosih__show_hud", { panel: "audit" }, ctx);
  ...
  it("updates memory and context panels", async () => {
    ...
    for (const panel of ["memory", "context"] as const) {
      const result = await handleInterfaceTool("nosih__show_hud", { panel }, ctx);
      const data = JSON.parse(result.content[0]!.text);
  ...
  it("emits hud.updated ledger event", async () => {
    const ctx = createCtx();
    await handleInterfaceTool("nosih__show_hud", { panel: "approvals" }, ctx);
  ...
  it("returns error when panel is missing", async () => {
    const ctx = createCtx();
    const result = await handleInterfaceTool("nosih__show_hud", {}, ctx);
    expect(result.content[0]!.text).toContain("panel is required");
  });
});

describe("nosih__notify", () => {
  ...
  it("sends a notification with action button", async () => {
    const ctx = createCtx();
    const result = await handleInterfaceTool("nosih__notify", {
  ...
  it("emits operator.notification ledger event", async () => {
    const ctx = createCtx();
    await handleInterfaceTool("nosih__notify", { message: "Alert" }, ctx);
  ...
  it("returns error when message is missing", async () => {
    const ctx = createCtx();
    const result = await handleInterfaceTool("nosih__notify", {}, ctx);
    expect(result.content[0]!.text).toContain("message is required");
  });
});

describe("nosih__interface_mode", () => {
  it("switches to each valid mode", async () => {
    const ctx = createCtx();
    for (const mode of ["hud", "compact", "voice-only", "silent"] as const) {
      const result = await handleInterfaceTool("nosih__interface_mode", { mode }, ctx);
      const data = JSON.parse(result.content[0]!.text);
  ...
  it("emits mode.changed ledger event", async () => {
    const ctx = createCtx();
    await handleInterfaceTool("nosih__interface_mode", { mode: "silent" }, ctx);
  ...
  it("returns error when mode is missing", async () => {
    const ctx = createCtx();
    const result = await handleInterfaceTool("nosih__interface_mode", {}, ctx);
    expect(result.content[0]!.text).toContain("mode is required");
  });
});

describe("nosih__store_memory", () => {
  it("stores with memoryBank present", async () => {
    const ctx = createCtx(true);
    const result = await handleInterfaceTool("nosih__store_memory", {
      key: "project-goal",
  ...
  it("returns error when key is missing", async () => {
    const ctx = createCtx();
    const result = await handleInterfaceTool("nosih__store_memory", { value: "x" }, ctx);
    expect(result.content[0]!.text).toContain("required");
  });

  it("returns error when value is missing", async () => {
    const ctx = createCtx();
    const result = await handleInterfaceTool("nosih__store_memory", { key: "k" }, ctx);
    expect(result.content[0]!.text).toContain("required");
  });
});

describe("nosih__recall_memory", () => {
  it("returns empty array without memoryBank", async () => {
    const ctx = createCtx(false);
    const result = await handleInterfaceTool("nosih__recall_memory", { query: "project" }, ctx);
  ...
  it("recalls matching entries from memoryBank", async () => {
    const ctx = createCtx(true);
    // Store first
    await handleInterfaceTool("nosih__store_memory", {
      key: "robot-task",
    ...
    // Recall
    const result = await handleInterfaceTool("nosih__recall_memory", {
      query: "robot",
      limit: 5,
    }, ctx);
  ...
  it("returns error when query is missing", async () => {
    const ctx = createCtx();
    const result = await handleInterfaceTool("nosih__recall_memory", {}, ctx);
...
describe("unknown interface tool", () => {
  it("returns unknown tool error", async () => {
    const ctx = createCtx();
    const result = await handleInterfaceTool("nosih__nonexistent_tool", {}, ctx);
