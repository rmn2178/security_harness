...
function printHelp(): void {
  console.log(`NOSIH Operator CLI (nosihctl)\n
Usage:
  nosihctl [global options] <group> <command> [command options]
...
Examples:
  nosihctl token issue --issuer <pub> --subject <pub> --resource ros2:///cmd_vel --actions publish --private-key <priv>
  nosihctl approvals list
  nosihctl approvals resolve --request-id <id> --status approved --by operator@site
  nosihctl ledger query --agent-id <pub> --limit 20
  nosihctl intercept run --agent-id <pub> --token-id <id> --resource ros2:///cmd_vel --action publish --params-json '{"twist":{"linear":0.2}}'
  nosihctl certify run --output docs/reports/standalone-conformance-certification.json
  nosihctl registry publish --token ./token.json --note "prod filesystem token"
  nosihctl registry list --issuer <pub>
  nosihctl registry list --resource mcp://filesystem
  nosihctl shipyard evidence export --output docs/reports/shipyard-evidence.jsonl
`);
}
...
run().catch((error: unknown) => {
  ...
  console.error(`nosihctl error: ${message}`);
  process.exit(1);
});
