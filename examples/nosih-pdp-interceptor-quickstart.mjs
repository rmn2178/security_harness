...
import { NOSIHPDPInterceptor } from "../packages/nosih-pdp-interceptor/dist/index.js";
...
const agentId = "did:key:z6Mkr7fExampleNOSIHAgent";
...
async function main() {
  const interceptor = new NOSIHPDPInterceptor({
    gateway: {
  ...
  console.log("NOSIH PDP Interceptor — 5 minute quickstart");
  ...
  const allowResult = await interceptor.runGuarded(allowRequest, {
    verifyGatePrerequisite: async () => ({
      ok: true,
      evidenceRef: "nosih://ledger/gate-allow-001",
    }),
    execute: async () => ({
      status: "ok",
      bytesWritten: 15,
