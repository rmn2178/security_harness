/**
 * NosihOS — unified entrypoint for the NOSIH Operating System.
 *
 * Orchestrates all NOSIH OS components:
 ...
 * @example
 * ```typescript
 * import { NosihOS } from "@pshkv/os-core";
 *
 * const os = new NosihOS({
...
import type { NosihOSConfig, NosihOSStatus } from "./types.js";
import {
  OpenClawAdapter,
...
export class NosihOS {
  private readonly config: NosihOSConfig;
  ...
  constructor(config: NosihOSConfig) {
    this.config = {
      openclawWsUrl: "ws://127.0.0.1:18789",
      ...config,
    };
  ...
  /**
   * Boot NOSIH OS — connect all components and verify health.
   */
  async boot(): Promise<{
    success: boolean;
  ...
  /**
   * Get full NOSIH OS status.
   */
  status(): NosihOSStatus {
    return {
