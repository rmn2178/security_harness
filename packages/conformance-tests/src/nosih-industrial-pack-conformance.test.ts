/**
 * NOSIH Industrial Pack conformance.
 *
...
const MANIFEST_PATH = resolve(REPO_ROOT, "nosih-industrial/manifest.v1.json");
...
describe("NOSIH Industrial Pack manifest v1", () => {
  const manifest = loadManifest();

  it("packages Sprint 3 under a real nosih-industrial root", () => {
    expect(manifest.packId).toBe("nosih-industrial-pack-v1");
    expect(manifest.schemaVersion).toBe("1.0.0");
    expect(manifest.status).toBe("preview");
    expect(manifest.root).toBe("nosih-industrial");
    expect(manifest.scopeNote).toContain("does not claim live certified");

    for (const path of Object.values(manifest.paths)) {
      expect(path.startsWith("nosih-industrial/")).toBe(true);
