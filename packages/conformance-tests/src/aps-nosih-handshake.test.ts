/**
 * NOSIH Protocol — APS-NOSIH-MCP Cross-Protocol Handshake Conformance Tests.
 *
 * Validates the three canonical scenarios from docs/specs/aps-nosih-handshake-v1.md:
 ...
 *
 * @module @nosih/conformance-tests/aps-nosih-handshake
 */
...
import { loadAPSNOSIHHandshakeFixture } from "./fixture-loader.js";
...
describe("APS-NOSIH-MCP Cross-Protocol Handshake — conformance", () => {
  ...
  it("fixture: aps-nosih-handshake.v1.json loads and has 3 cases covering scenarios A, B, C", () => {
    const fixture = loadAPSNOSIHHandshakeFixture();
    expect(fixture.fixtureId).toBe("aps-nosih-handshake-v1");
    expect(fixture.interopProtocol).toBe("APS-NOSIH-MCP");
  ...
  it("Scenario C: cascade revocation — child token denied after explicit revocation", async () => {
    ...
    const r0Req: NosihCapabilityTokenRequest = {
      issuer: root.publicKey,
    ...
    // C1 is NOT yet revoked — NOSIH checks tokens independently
  ...
  it("Scenario C: cascade gap — child token NOT denied if only parent is revoked (operator must cascade)", async () => {
    ...
    // C1 should still be allowed — NOSIH does not auto-walk the chain
  ...
  it("Delegation depth: chain at max depth (3) is accepted; depth 4 is rejected", () => {
    ...
    const t0Req: NosihCapabilityTokenRequest = {
      issuer: root.publicKey,
  ...
  it("Monotonic narrowing: delegated child inherits parent resource (cannot be independently broadened)", () => {
    ...
    const parentReq: NosihCapabilityTokenRequest = {
      issuer: root.publicKey,
