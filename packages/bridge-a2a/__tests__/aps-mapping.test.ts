/**
 * APS ↔ NOSIH interoperability mapping tests.
 */

import { describe, it, expect } from "vitest";
import { apsScopeToNosihMapping, nosihTokenToApsProjection } from "../src/aps-mapping.js";
...
describe("apsScopeToNosihMapping", () => {
  it("maps single-domain APS scope to scoped a2a URI", () => {
    ...
    const result = apsScopeToNosihMapping(scope);
  ...
  it("maps APS allowedActions to NOSIH a2a.* actions", () => {
    const scope: ApsDelegationScope = {
      resourceScope: ["logistics:dispatch"],
  ...
  it("maps APS temporalValidity to NOSIH timeWindow", () => {
...
describe("nosihTokenToApsProjection", () => {
  ...
  it("NOSIH tokens are always attestation grade 2 (infrastructure-attested)", () => {
    const token = makeToken({});
    const projection = nosihTokenToApsProjection(token);
  ...
  it("preserves the full nosihPhysicalConstraints for APS to log", () => {
    const token = makeToken({ maxVelocityMps: 0.5, maxForceNewtons: 50 });
    const projection = nosihTokenToApsProjection(token);
    expect(projection.nosihPhysicalConstraints.maxVelocityMps).toBe(0.5);
    expect(projection.nosihPhysicalConstraints.maxForceNewtons).toBe(50);
  });

  it("attenuation invariant: NOSIH projection scope ⊆ original token scope", () => {
    const token = makeToken({
