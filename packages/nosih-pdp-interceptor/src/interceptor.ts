import type { ISO8601, PolicyDecision, NosihRequest, UUIDv7 } from "@pshkv/core";
...
function denyFromError(
  ...
  const decision: PolicyDecision = {
    ...
    denial: {
      ...
      suggestedAlternative: "Retry once the NOSIH policy gateway is healthy.",
    },
  };
...
/**
 * Thin adapter that presents a SEP-1763-style PDP interface on top of NOSIH's PolicyGateway.
 */
export class NOSIHPDPInterceptor {
  readonly type = "policy-pdp" as const;

  private readonly gateway: NOSIHPDPInterceptorConfig["gateway"];
  ...
  constructor(config: NOSIHPDPInterceptorConfig) {
  ...
  async evaluate(request: PDPInterceptorRequest): Promise<PDPDecision> {
    ...
    if (!tokenId) {
      throw new Error(
        "NOSIHPDPInterceptor requires a tokenId either in config.defaultTokenId or request.context.tokenId",
      );
    }
    ...
    try {
      ...
      const reason =
        error instanceof Error
          ? `NOSIH gateway unavailable: ${error.message}`
          : "NOSIH gateway unavailable";
