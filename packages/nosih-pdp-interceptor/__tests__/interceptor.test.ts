...
import { NOSIHPDPInterceptor } from "../src/interceptor.js";
...
describe("NOSIHPDPInterceptor", () => {
  it("maps a SEP-1763 style request into a NOSIH gateway request", async () => {
    let captured: NosihRequest | undefined;
  ...
  it("returns failed when downstream execution throws", async () => {
    ...
    const result = await interceptor.runGuarded(
      ...
      },
      {
        verifyGatePrerequisite: async () => ({
          ok: true,
          evidenceRef: "nosih://ledger/gate-001",
        }),
        execute: async () => {
