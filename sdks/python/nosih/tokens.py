"""
NOSIH Protocol — Capability Token request builder.
...
``packages/capability-tokens/src/issuer.ts``:
  ...
  3. Gateway validates, signs with the root Ed25519 key, and returns the
     complete :data:`NosihCapabilityToken`.

Usage::

    from datetime import datetime, timezone, timedelta
    from nosih.tokens import build_token_request
...
class PhysicalConstraints:
    ...
    Mirrors ``NosihPhysicalConstraints`` from
...
class DelegationChain:
    ...
    Mirrors ``NosihDelegationChain`` from
    ``packages/core/src/types/capability-token.ts``.
    """
...
class CapabilityTokenRequest:
    ...
    Mirrors ``NosihCapabilityTokenRequest`` from
    ``packages/core/src/types/capability-token.ts``.
