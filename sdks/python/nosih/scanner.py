"""
NOSIH Protocol — MCPScanner.

Python port of the ``@nosih/bridge-mcp`` MCP scanner CLI.

Classifies MCP tool definitions into NOSIH approval tiers using the same
keyword and annotation logic as the TypeScript ``mcp-resource-mapper.ts``.

CLI usage::

    nosih-scan --server my-server tool1 tool2 ...
    nosih-scan --server my-server --json tool1 tool2 ...
    nosih-scan --file tools.json

Programmatic usage::

    from nosih.scanner import scan_tool, scan_server
...
class ServerScanReport:
    ...
    def format_report(self, *, no_color: bool = False) -> str:
        ...
        lines.append(bold(f"\nNOSIH MCP Scanner — {self.server_id}"))
...
def scan_tool(
    ...
    """Classify a single MCP tool into a NOSIH approval tier.
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="nosih-scan",
        description="NOSIH MCP Scanner — classify MCP tool risk tiers",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  nosih-scan --server filesystem readFile writeFile deleteFile bash
  nosih-scan --server filesystem --json readFile writeFile
  nosih-scan --file tools.json
  nosih-scan --server myserver --fail-on HIGH toolA toolB
...
def main(argv: list[str] | None = None) -> None:
    """CLI entry point for ``nosih-scan``."""
