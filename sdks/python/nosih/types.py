"""
NOSIH Protocol — Pydantic models mirroring TypeScript types.

Covers:
  - ApprovalTier / RiskTier enums
  - NosihRequest  (policy.ts → NosihRequest)
  - PolicyDecision (policy.ts → PolicyDecision)
  - LedgerEvent  (evidence.ts → NosihLedgerEvent)
...
class NosihRequest(BaseModel):
    """A request entering the Policy Gateway for evaluation.

    Maps to the TypeScript ``NosihRequest`` interface in
...
class LedgerEvent(BaseModel):
    ...
    Maps to the TypeScript ``NosihLedgerEvent`` interface in
...
class GatewayConfig(BaseModel):
    ...
    base_url: str = Field(..., description="Base URL of the NOSIH Policy Gateway, e.g. http://localhost:3000")
