"""
NOSIH Protocol — OpenAI Agents SDK governance adapter.

Provides a thin runtime governance layer for OpenAI Agents SDK tool calls:
- pre-tool authorization via NOSIH gateway intercept
...
from .types import LedgerEvent, PolicyDecision, NosihRequest


class NosihGovernanceError(Exception):
    """Base exception for NOSIH OpenAI Agents governance adapter failures."""


class NosihDeniedError(NosihGovernanceError):
    ...
    def __init__(self, decision: PolicyDecision) -> None:
        ...
        super().__init__(f"NOSIH denied request: {policy} — {reason}")
        self.decision = decision
        self.reason = reason
        self.policy_violated = policy


class NosihApprovalRequiredError(NosihGovernanceError):
    """Raised when a request requires T2/T3 approval and no resolver is provided."""

    def __init__(self, decision: PolicyDecision, request: NosihRequest) -> None:
        reason = decision.escalation.reason if decision.escalation else "Approval required"
        super().__init__(f"NOSIH escalation required: {reason}")
        self.decision = decision
...
class NosihApprovalTimeoutError(NosihGovernanceError):
    ...
    def __init__(self, approval_request_id: str | None, timeout_s: float) -> None:
        ...
        super().__init__(f"NOSIH approval timed out (request={req}, timeout_s={timeout_s})")
        self.approval_request_id = approval_request_id
        self.timeout_s = timeout_s


class NosihApprovalDeniedError(NosihGovernanceError):
    ...
    def __init__(self, approval_request_id: str | None, reason: str | None = None) -> None:
        ...
        super().__init__(f"NOSIH approval denied (request={req}){suffix}")
        self.approval_request_id = approval_request_id
        self.reason = reason
...
class OpenAIAgentsGovernanceAdapter:
    ...
    runtime that constructs a `NosihRequest` per call.
    """
    ...
    async def authorize_tool_call(
        ...
        *,
        ...
        """
        Authorize a tool call through NOSIH and resolve escalations when possible.

        Behavior:
        - allow/transform => returns PolicyDecision
        - deny => raises NosihDeniedError
        - escalate:
          - with resolver => resolve approval and return decision on approved
          - without resolver => raise NosihApprovalRequiredError
          - with timeout => fail-closed denial on timeout + raise NosihApprovalTimeoutError
        """
        ...
        if decision.action == "deny":
            raise NosihDeniedError(decision)
        ...
        if on_escalation is None:
            if approval_timeout_s is not None:
                ...
                raise NosihApprovalTimeoutError(decision.approval_request_id, approval_timeout_s)
            raise NosihApprovalRequiredError(decision, request)
        ...
        if resolution is None:
            ...
            raise NosihApprovalTimeoutError(decision.approval_request_id, timeout_s)
        ...
        if resolution.status != "approved":
            raise NosihApprovalDeniedError(decision.approval_request_id, resolution.reason)
