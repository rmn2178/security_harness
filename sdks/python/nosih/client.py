"""
NOSIH Protocol — GatewayClient.

Async HTTP client for the NOSIH Policy Gateway REST API.
...
from .types import GatewayConfig, LedgerEvent, PolicyDecision, NosihRequest
...
class GatewayClient:
    """Async NOSIH Policy Gateway client.

    Parameters
    ----------
    config:
        A :class:`~nosih.types.GatewayConfig` instance describing the gateway
    ...
    async def intercept(self, request: NosihRequest) -> PolicyDecision:
        """POST /v1/intercept — evaluate a single NOSIH request.

        Parameters
        ----------
        request:
            The :class:`~nosih.types.NosihRequest` to evaluate.
    ...
    async def intercept_batch(self, requests: list[NosihRequest]) -> list[PolicyDecision]:
        """POST /v1/intercept/batch — evaluate multiple requests atomically.

        Returns a list of :class:`~nosih.types.PolicyDecision` objects in the
    ...
    async def issue_token(self, token_request: dict[str, Any]) -> dict[str, Any]:
        ...
        See :func:`nosih.tokens.build_token_request` to build the request body.
        """
    ...
    async def discovery(self) -> dict[str, Any]:
        """GET /.well-known/nosih.json — gateway discovery document."""
        return await self._get("/.well-known/nosih.json")  # type: ignore[return-value]
