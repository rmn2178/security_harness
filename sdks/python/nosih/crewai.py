"""
NOSIH Protocol — CrewAI GuardrailProvider compatibility adapter.

Maps NOSIH gateway decisions to a simple pre-tool-call contract:
  allow | deny | escalate
...
from .types import ApprovalTier, PolicyDecision, NosihRequest
...
class CrewAIGuardrailProviderCompat:
    ...
    async def pre_tool_call(
        ...
        *,
        ...
        """
        Evaluate a request through NOSIH and return allow/deny/escalate.
        ...
        if decision.action == "deny":
            return GuardrailDecision(
                action="deny",
                reason=decision.denial.reason if decision.denial else "Denied by NOSIH policy",
