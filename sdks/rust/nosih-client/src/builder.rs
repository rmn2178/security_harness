//! Fluent builder for [`NosihRequest`].

use crate::types::{PhysicalContext, NosihRequest};
use uuid::Uuid;

/// Fluent builder for constructing a [`NosihRequest`].
///
/// Auto-generates `request_id` (UUIDv4) and `timestamp` (ISO 8601) on [`build`](Self::build).
pub struct NosihRequestBuilder {
...
impl Default for NosihRequestBuilder {
    fn default() -> Self {
        Self::new()
    }
}
