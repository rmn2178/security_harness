use crate::{builder::NosihRequestBuilder, error::Result, retry, retry::RetryConfig, types::*};
use reqwest::Client;
use serde_json::Value;

/// Configuration for the NOSIH client.
pub struct NosihClientConfig {
    pub gateway_url: String,
    pub api_key: Option<String>,
    pub retry_config: RetryConfig,
}

impl Default for NosihClientConfig {
    fn default() -> Self {
        Self {
...
/// Async NOSIH gateway client.
pub struct NosihClient {
    http: Client,
    config: NosihClientConfig,
}

impl NosihClient {
    pub fn new(config: NosihClientConfig) -> Self {
        Self {
            http: Client::new(),
            config,
        }
    ...
    pub async fn intercept(&self, request: &NosihRequest) -> Result<PolicyDecision> {
        let resp = self
        ...
        if !resp.status().is_success() {
            ...
            let message = body
                ...
                .to_string();
            return Err(crate::error::NosihError::Gateway { status, message });
        }
    ...
    pub async fn intercept_with_retry(&self, request: &NosihRequest) -> Result<PolicyDecision> {
    ...
    pub async fn issue_token(&self, req: &IssueTokenRequest) -> Result<NosihCapabilityToken> {
        let resp = self
        ...
        Ok(resp.json::<NosihCapabilityToken>().await?)
    }
    ...
    pub fn request(&self) -> NosihRequestBuilder {
        NosihRequestBuilder::new()
    }
}
